...
from struct import *
#Obtiene el valor de los flags IP, NOTA: debido al formato dado por el auxiliar, no sera usado
def getFlagsIP(data):
...
#   get the 3rd bit and shift right
    ...
    return flags
#Obtiene el valor de las flags del Header TCP
def getFlagsTCP(packet):
    ...
    new_line = '\n\t|-'
    Flags = '\t|-' + FlagTCP_URG[URG] + new_line + FlagTCP_ACK[ACK] + new_line + FlagTCP_PSH[PSH] + new_line + FlagTCP_RST[RST] + new_line + FlagTCP_SYN[SYN] + new_line + FlagTCP_FIN[FIN]
    return Flags
...
class Sniffer:
    ...
    def capture_packets(self):
        while True:
            ...
            print("\n----------------------Sniffer Start-----------------------")
            # print raw packet if noraw not given Nota: Debido al formato del auxiliar no se mostrara lo siguiente
            #if self.args.noraw:
                #print('Packet: {}'.format(str(packet)))
            #Muestra los datos del estudiante
            print('Estudiante: Cristhian Alejandro Larico Quisbert..............Ci: 9165912 LP')
            print('Paralelo: "B"')
            ...
            headerinfo = [
                ...
                '\t|-IP Header Length (IHL) : {}'.format(ihl)+' DWORDS or '+str(ihl*32//8) +' Bytes',
                ...
                '\t|-Source IP              : {}'.format(s_addr),
                '\t|-Destination IP         : {}'.format(d_addr)]
